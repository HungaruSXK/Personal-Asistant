...
@bot.tree.command(name="cancelclockin", description="Cancel your current clock-in status")
async def cancelclockin(interaction: discord.Interaction):
    if not is_admin(interaction.user.id):
        return await interaction.response.send_message("⛔ Only you may use the time clock.", ephemeral=True)
    uid = str(interaction.user.id)
    if uid not in data["clock_state"]:
        return await interaction.response.send_message("⚠️ You are not currently clocked in.", ephemeral=True)

    data["clock_state"].pop(uid)
    save_data()
    await interaction.response.send_message("🔄 Clock-in status cancelled. You can now clock in again.", ephemeral=True)
...
async def help_cmd(interaction: discord.Interaction):
    ...
    embed.add_field(name="🕒 Clock (WITA/Bali)", value="`/clockin` from 08:00 · `/clockout` · `/cancelclockin` reset · overtime DM after 18:00 · `/timesheet` · `/cleartimesheet` (Admin)", inline=False)
